...
FN = strcat('PINGING_TypesQIF11Tau04BifDiagFullVaryPhiEVaryPhiI', num2str(cur_fileID), '.mat');
if (exist(FN, 'file') == 2)
    load(FN);
    Begin_E_f_i = E_f_i + 1;
else
...
for E_f_i = Begin_E_f_i:1:N_OneoverThetaPhiE_lin
    ...
    for OneoverThetaPhiI = OneoverThetaPhiI_lin
        [status, result] = system(strcat('top -n 1 -b -u viriyopa > PINGING_TypesQIF11Tau04BifDiagFullVaryPhiEVaryPhiI', num2str(cur_fileID), '.log'));
