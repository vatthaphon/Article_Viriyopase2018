...
root_txt = '/Volumes/Data/paper2_Raoul/Sim_two_neurons_Raoul/Types12BifDiagFullVaryPhiEPhiI';
LW = 20;
...
longest_duration_to_spike = -1000;
longest_duration_to_spike_thetaPhiI = NaN;
longest_duration_to_spike_thetaPhiE = NaN;
for i = 1:1:10
                ...
                for jjj = 1:1:N_OneoverThetaPhiI_lin
                    ...
                    if (isnan(tmp_deltaPsi(jjj, jj)) == 0)
                        if ((j == 5) && (k == 1))
                            ...
                            if (OneoverThetaPhiE_lin(jj) <= 0.77)
                                % Determine when do they spike after the arrival of
                                % the E input.
                                deltaPsi_after_sce5 = cal_delta_psi_after_scenario5(gammaE, gammaI, thetaVE, thetaVI, 1/OneoverThetaPhiE_lin(jj), 1/OneoverThetaPhiI_lin(jjj), tmp_deltaPsi(jjj, jj), tau, epsilonEI, epsilonIE);
                                duration_to_spike = 1/OneoverThetaPhiI_lin(jjj) + (deltaPsi_after_sce5 + (1/OneoverThetaPhiE_lin(jj) - 1/OneoverThetaPhiI_lin(jjj)) - tau);
                                ...
                                if ((duration_to_spike/tmp_T) > longest_duration_to_spike)
                                    longest_duration_to_spike = duration_to_spike/tmp_T;
                                    longest_duration_to_spike_thetaPhiI = OneoverThetaPhiI_lin(jjj);
                                    longest_duration_to_spike_thetaPhiE = OneoverThetaPhiE_lin(jj);
                                end
                            end
                        end
                    else
...
longest_duration_to_spike
longest_duration_to_spike_thetaPhiI
longest_duration_to_spike_thetaPhiE
...
% maximize_a_fig(gcf);
% % m_savefig('Types12Tai04BifDiagFullVaryPhiEVaryPhiI_v4', 'eps');
% % m_savefig('Types12Tai04BifDiagFullVaryPhiEVaryPhiI_v4', 'png');
% m_savefig('Types12Tai04BifDiagFullVaryPhiEVaryPhiI_v5', 'eps');
