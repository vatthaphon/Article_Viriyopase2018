...
root_txt = 'E:\paper2_Raoul\Sim_two_neurons_Raoul\Types11BifDiagFullVarygI2E';

figure(1); hold on;
%% Plot ING
for i = 0:1:9
    
    file_txt = strcat(root_txt, '\v0\ING\ING_Types11BifDiagFullVarygI2E', num2str(i), '.mat');
    load(file_txt);
    
%     plot(epsilonEI_lin, f1, 'r*', 'LineWidth', 4);
%     plot(epsilonEI_lin, f2, 'g*', 'LineWidth', 4);
%     plot(epsilonEI_lin, f3, 'b*', 'LineWidth', 4);
%     plot(epsilonEI_lin, f4, 'k*', 'LineWidth', 4);
%     plot(epsilonEI_lin, f5, 'y*', 'LineWidth', 4);

    if (i == 0)
        epsilonEI_total = epsilonEI_lin;
        f1_total = f1;
        f2_total = f2;
        f3_total = f3;
        f4_total = f4;
        ...
        epsilonEI_total = [epsilonEI_total epsilonEI_lin];
...
% plot(epsilonEI_total, f2_total, 'g*');
% plot(epsilonEI_total, f3_total, 'b*');
% plot(epsilonEI_total, f4_total, 'k*');
% % plot(epsilonEI_total, f5_total, 'y*');
% 
% Filter
N_filter = 10;
i_filter = 0;
[rows, cols] = size(epsilonEI_total);
for i = 1:1:cols
    if (cmp(rem(i, N_filter), 0, 1e-6) == 0)
        i_filter = i_filter + 1;
        epsilonEI_filter(1, i_filter) = epsilonEI_total(1, i);
...
% Combine
[rows, cols] = size(epsilonEI_filter);
...
for i = 1:1:cols
    if (isnan(f2_filter(1, i)) == 0)
        ...
        epsilonEI_comb(1, i_comb) = epsilonEI_filter(1, i);
...
plot(epsilonEI_comb, f_comb, 'b-', 'LineWidth', 4);
...
m_tau = 0.4;
epsilonEI = linspace(-1.0, 0, 100);       % From I to E

m_H = -log(exp(-2.*m_tau) - (1 - exp(-thetaPhiE)).*epsilonEI);
T = 2.*m_tau + thetaPhiE - m_H;

plot(epsilonEI, epsilonEI./epsilonEI.*1./T, 'r-', 'LineWidth', 4)

%% Plot PINGING
for i = 0:1:9
    file_txt = strcat(root_txt, '\v0\PINGING\PINGING_Types11BifDiagFullVarygI2E', num2str(i), '.mat');
    load(file_txt);
    
    load(file_txt);
    
%     plot(epsilonEI_lin, f1, 'ro', 'LineWidth', 4);
%     plot(epsilonEI_lin, f2, 'go', 'LineWidth', 4);
%     plot(epsilonEI_lin, f3, 'bo', 'LineWidth', 4);
%     plot(epsilonEI_lin, f4, 'ko', 'LineWidth', 4);
%     plot(epsilonEI_lin, f5, 'yo', 'LineWidth', 4);
...
% % plot(epsilonEI_total, f1_total, 'r*');
% plot(epsilonEI_total, f2_total, 'g-', 'LineWidth', 4);
% plot(epsilonEI_total, f3_total, 'k-', 'LineWidth', 4);
% plot(epsilonEI_total, f4_total, 'LineWidth', 4, 'Color', [101/255 163/255 92/255]);
% % plot(epsilonEI_total, f5_total, 'y*');
% 
% Filter
N_filter = 1;
...
for i = 1:1:cols
    ...
    if (isnan(f4_filter(1, i)) == 0)
        ...
        epsilonEI_comb1(1, i_comb1) = epsilonEI_filter(1, i);
        f_comb1(1, i_comb1) = f4_filter(1, i);
    end    
end

plot(epsilonEI_comb, f_comb, 'g-', 'LineWidth', 4);
plot(epsilonEI_comb1, f_comb1, 'LineWidth', 4, 'Color', [101/255 163/255 92/255]);
...
set(gca,'XTick',[-0.8 -0.6 -0.4 -0.2],'XTickLabel',{'';'';'';''});
set(gca,'YTick',[0.34 0.36 0.38 0.4 0.42],'YTickLabel',{'';'';'';''});

maximize_a_fig(gcf);
ylim([0.335 0.43]);
xlim([-0.8 -0.2]);
% m_savefig('Types11BifDiagFullVarygI2E', 'eps');
end
