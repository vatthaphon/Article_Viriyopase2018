function plotplotTypes12BifDiagFullVarygI2E
clc;
clear all;
close all;

root_txt = 'E:\paper2_Raoul\Sim_two_neurons_Raoul\Types12BifDiagFullVarygI2E';

figure(1); hold on;
%% Plot PINGING
for i = 0:1:9        
    file_txt = strcat(root_txt, '\PINGING\v0\PINGING_Types12Tau04BifDiagFullVarygI2E', num2str(i), '.mat');
    load(file_txt);
    
%     plot(epsilonEI_lin, f1, 'r*');
%     plot(epsilonEI_lin, f2, 'g*');
%     plot(epsilonEI_lin, f3, 'b*');
%     plot(epsilonEI_lin, f4, 'k*');
%     plot(epsilonEI_lin, f5, 'c*');
%     plot(epsilonEI_lin, f51, 'y*');
%     plot(epsilonEI_lin, f15, 'o');
...
% plot(epsilonEI_total, f51_total, 'm*');
% plot(epsilonEI_total, f15_total, 'co');
...
%% Plot ING
for i = 0:1:9
    file_txt = strcat(root_txt, '\ING\v0\ING_Types12Tau04BifDiagFullVarygI2E', num2str(i), '.mat');
    load(file_txt);
...
m_H = -log(exp(-2.*m_tau) - (1 - exp(-thetaPhiE)).*epsilonEI_total);
T = 2.*m_tau + thetaPhiE - m_H;

plot(epsilonEI_total, epsilonEI_total./epsilonEI_total.*1./T, 'r-', 'LineWidth', 4)
...
set(gca,'XTick',[-0.28 -0.24 -0.20 -0.16 -0.12],'XTickLabel',{'';'';'';'';''});
set(gca,'YTick',[0.59 0.61 0.63 0.65],'YTickLabel',{'';'';'';''});

maximize_a_fig(gcf);
ylim([0.58 0.66]);
xlim([-0.28 -0.12]);
m_savefig('Types12BifDiagFullVarygI2E', 'eps');
